...
- **`ContextSnapshotEvent` is silent until a context window is known** — the CLI reports the window only in the result event's `modelUsage` and in the `get_context_usage` control response, never on the mid-turn `message_start`. Verified against CLI 2.1.263 over three live runs. Rather than emit a measurement against a zero window (which renders as a full or empty meter, both wrong), the event is withheld, so a Session's first turn produces none. `QueryContextUsage()` once, at connect, removes the blind spot; `ParseEvents` cannot (it returns at the terminal result event).
