...
// npm on Windows installs the CLI behind a cmd.exe shim (`claude.cmd`), so
...
// arguments cmd.exe cannot safely escape (the CVE-2024-24576 hardening — a
// `--append-system-prompt` containing `%` or `"` fails at Start), and the
// shim puts a cmd.exe layer into the process tree. findShimCLIJS resolves
// the shim to the entry script it wraps so the executor can run node
// directly, exactly as the shim itself would.
//
// The logic lives in a platform-neutral file so it stays testable off
// Windows; only executor_windows.go calls it.

// findShimCLIJS reports the CLI entry script an npm shim wraps. It trusts
...
func findShimCLIJS(shimPath string) (string, bool) {
	...
	for _, rel := range []string{
		...
		cliJS := filepath.Join(pkgDir, filepath.FromSlash(entry))
		if info, err := os.Stat(cliJS); err == nil && !info.IsDir() {
			return cliJS, true
		}
	}
	return "", false
...
// is either a bare string or a name→script map; "cli.js" is the fallback
// when the field is missing or unreadable, matching the package's layout.
